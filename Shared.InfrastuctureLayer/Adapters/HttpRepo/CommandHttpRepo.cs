...
namespace Shared.InfrastuctureLayer.Adapters.HttpRepo
{
    public class CommandHttpRepo : ICommandGenericMethodRepo
    {
        ...
        public async Task<Response> InsertAsync<TEntityDto>(TEntityDto entityDto) where TEntityDto : class, IDbEntity<TEntityDto>, new()
        {
            ...
            if (_httpClient is not null)
            {
                try
                {
                    ...
                    if (httpResponse.StatusCode == HttpStatusCode.BadRequest)
                    {
                        ...
                        if (response is null)
                        {
                            defaultResponse.SetNewError("A mentés http kérés hibát okozott!");
                        }
            ...
            defaultResponse.SetNewError("Az adatok mentése nem lehetséges!");
        ...
        public async Task<Response> UpdateAsync<TEntityDto>(TEntityDto entityDto) where TEntityDto : class, IDbEntity<TEntityDto>, new()
        {
            ...
            if (_httpClient is not null)
            {
                try
                {
                    ...
                    if (httpResponse.StatusCode == HttpStatusCode.BadRequest)
                    {
                        ...
                        if (response is null)
                        {
                            defaultResponse.SetNewError("A módosítás http kérés hibát okozott!");
                        }
                        else return response;
                    }
                    else if (httpResponse.StatusCode == HttpStatusCode.InternalServerError)
            ...
            defaultResponse.SetNewError("Az adatok frissítés nem lehetséges!");
            return defaultResponse;
        }
        ...
        public async Task<Response> DeleteAsync<TEntityDto>(Guid id) where TEntityDto : class, IDbEntity<TEntityDto>, new()
        {
            ...
            if (_httpClient is not null)
            {
                try
                {
                    ...
                    if (httpResponse.StatusCode == HttpStatusCode.BadRequest)
                    {
                        ...
                        if (response is null)
                        {
                            defaultResponse.SetNewError("A törlés http kérés hibát okozott!");
                        }
                        else return response;
                    }
                    else if (httpResponse.StatusCode == HttpStatusCode.InternalServerError)
                    ...
                    else
                    {
                        ...
                        if (response is null)
                        {
                            defaultResponse.SetNewError("A { http kérés hibát okozott!");
                        }
                        else return response;
                    }
                }
            ...
            defaultResponse.SetNewError("Az adatok törlése nem lehetséges!");
            return defaultResponse;
        }
